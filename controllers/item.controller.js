...
const add = async (req, res) => {
  ...
  if ([token, title, type, salePrice, category, unit].some(field => !field || field === "")) {
  ...
  try {
    ...
    if (update && id) {
      const update = await itemModel.updateOne({ _id: id }, {
        $set: {
          title, type, salePrice, category, details, unit
        }
      })
    ...
    const insert = await itemModel.create({
      ...
      title, type, salePrice, category, details, unit, stock: openingStock, alert: stockAlert
    });
