...
const add = async (req, res) => {
  ...
  if ([token, name, type, contactNumber, billingAddress, pan, gst,]
  ...
  try {
    ...
    const isPartyExist = await partyModel.findOne({
      userId: getInfo._id, companyId: getUserData.activeCompany, gst: gst, isDel: false
    });
    ...
    if (update && id) {
      const update = await partyModel.updateOne({ _id: id }, {
        $set: {
          ...
          pan, gst, openingBalance, details, partyCategory, 
