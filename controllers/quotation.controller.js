...
const add = async (req, res) => {
  ...
  if ([token, party, quotationNumber, estimateDate, validDate, items]
  ...
  try {
    ...
    if (update && id) {
      const update = await quotationModel.updateOne({ _id: id }, {
        $set: {
          party, quotationNumber, estimateDate, validDate, items,
...
const filter = async (req, res) => {
  ...
  let allData = await quotationModel.find({...query, isDel: false}).skip(skip).limit(limit).sort({ _id: -1 }).populate('party');
